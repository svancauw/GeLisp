...
;;(load "/Users/saschavancauwelaert/Documents/EPL/PhD/code/GeLisp/examples/test.lisp")
;;(test)

(load "/Users/saschavancauwelaert/Documents/EPL/PhD/code/GeLisp/gelisp_highlayer.lisp")
...
(defun testGraph ()
	...
	(setq f (GlSpace_newGraphVar_int sp 2))
	
	(setq g (GlSpace_newGraphVar_int sp 2))
	
	;WORKS (right tuples are unknown)
...
(defun testCPRel ()
	...
	(setq lubA (new_GRelation 2))
	
	(setq glbA (new_GRelation 2))
	
	(GRelation_add lubA (new_Tuple_binary 1 3))
	
	(setq lubB (new_GRelation 2))
	
	(setq glbB (new_GRelation 2))
	
	(GRelation_add lubB (new_Tuple_binary 1 3))
	
	(setq relA (GlSpace_newCPRelVar sp glbA lubA)) ; OK !!!
	
	(setq relB (GlSpace_newCPRelVar sp glbB lubB))
	
	(setq relC (GlSpace_newCPRelVar sp glbB lubB))
		
	;(setq relList (new_CPRelVarList sp 5 glb lub))
	
	(GlSpace_debug sp)
	
	;(GRelation_arity (CPRelVar_glb relA))
	;(GRelation_arity (CPRelVar_glb relB))
	
	;works !!!
	;(equal_rel sp relA relB)
	
	;works!!!
	;(comp_rel sp relA relB)
	...
	(setq sol_relA (CPRelVar_glb (GlSpace_getCPRelVar sol (GlCPRelVar_getIndexInVector relA))))
	
	(setq iter_sol_relA (new_GRelationIter sol_relA))
	
	(setq tA1 (GRelationIter_val iter_sol_relA))
	
	(Tuple_at tA1 0); works !!!
		
	(GRelationIter___funcall__ iter_sol_relA) ; works !!!
		
	(GRelationIter___preincr__ iter_sol_relA) ; works !!!
		
		
		;;;;;;;;;;;;;;;;;;; ternary relations : works !!!
	
	(setq sp (new_GlSpace))
		
	(setq glbA (new_GRelation 3))
		
	(setq lubA (new_GRelation 3))
	
	(GRelation_add lubA (new_Tuple_ternary 1 2 3))
;	(GRelation_add glbA (new_Tuple_ternary 1 2 3))

	(setq relA (GlSpace_newCPRelVar sp glbA lubA))
	
	(setq glbB (new_GRelation 3))
		
	(setq lubB (new_GRelation 3))
	
	(GRelation_add lubB (new_Tuple_ternary 4 5 6))

	(setq relB (GlSpace_newCPRelVar sp glbB lubB))
	
	(setq glbC (new_GRelation 3))
		
	(setq lubC (new_GRelation 3))
	
	(GRelation_add lubC (new_Tuple_ternary 1 2 3))
	(GRelation_add lubC (new_Tuple_ternary 4 5 6))
	
	(setq relC (GlSpace_newCPRelVar sp glbC lubC))
	
	;(union_rel sp relA relB relC); works !
	
	;(intersect sp relC relB relB); works!
		
	;(subset sp relA relB);works !!!
		
	;(disjoint sp relA relB)	;works!!!
